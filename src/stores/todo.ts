...
import { canUseStorage, readJson, writeJson } from '../utils/storage'
import type {
...
interface TodoPersistedStateLegacy {
  tasks?: Task[]
  projects?: Project[]
}
...
export const useTodoStore = defineStore('todo-list', () => {
  ...
  function migrateState(
    parsed: Partial<TodoPersistedState> & TodoPersistedStateLegacy,
  ): TodoPersistedState | null {
    const version = parsed.version ?? 0
    if (version !== STORAGE_VERSION && version !== 0) {
      return null
    }
  ...
  function loadState(): void {
    ...
    const parsed = readJson<Partial<TodoPersistedState> & TodoPersistedStateLegacy>(STORAGE_KEY)
    if (!parsed) {
      projects.value = normalizeProjects([])
      return
    }
    ...
    try {
      ...
      const safeProjects = normalizeProjects(migrated.projects)
      const projectIds = new Set(safeProjects.map((item) => item.id))

      projects.value = safeProjects
      tasks.value = normalizeTasks(migrated.tasks, projectIds)
    } catch {
      projects.value = normalizeProjects([])
      tasks.value = []
    }
  ...
  function setUndoSnapshot(message: string): void {
    undoSnapshot = {
  ...
  function maybeSendDueNotifications(): void {
  ...
  function resequenceManualOrder(): void {
  ...
  function setTaskCompleted(taskId: string, completed: boolean): boolean {
    const index = tasks.value.findIndex((item) => item.id === taskId)
    if (index === -1) {
      return false
    }
  ...
  function toggleTaskCompleted(taskId: string): void {
  ...
  function toggleSubtaskCompleted(taskId: string, subtaskId: string): void {
  ...
  function setSubtaskPlannedAt(taskId: string, subtaskId: string, plannedAt: string | null): boolean {
    const taskIndex = tasks.value.findIndex((item) => item.id === taskId)
    if (taskIndex === -1) {
      return false
    }
  ...
  function setSubtaskText(taskId: string, subtaskId: string, text: string): boolean {
    const cleaned = text.trim()
    if (!cleaned) {
      return false
    }
  ...
  function reorderTasks(draggedTaskId: string, targetTaskId: string): void {
  ...
  function sortTasksByDueDate(): boolean {
  ...
  function sortTasksByPriorityAndDueDate(): boolean {
    if (tasks.value.length <= 1) {
      return false
    }
